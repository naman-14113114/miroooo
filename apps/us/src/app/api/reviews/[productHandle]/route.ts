...
export async function GET(
  ...
  const filteredReviews = sortReviews(
    dataset.reviews.filter((review) => {
      ...
      // Current public placeholder reviews are all verified, so this flag is a no-op
...
export async function POST(
  ...
  if (productHandle === "miroooo-electric-toothbrush-x2") {
    revalidatePath("/products/miroooo-electric-toothbrush-x2");
    revalidatePath("/products/miroooo-electric-toothbrush-x2");
  } else {
