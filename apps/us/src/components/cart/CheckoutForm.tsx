...
export function CheckoutForm({ initialCustomer }: CheckoutFormProps) {
  ...
  const maskQuantity =
    lines.find(
      (line) => line.type === "product" && line.productId === "miroooo-electric-toothbrush-x2",
  ...
  async function handleCheckout() {
    ...
    window.dispatchEvent(
      new CustomEvent("store:started-checkout", {
        detail: {
          lines,
          totals,
        },
    ...
    try {
      ...
      setIsRedirecting(false);
      setError("Checkout is unavailable until this store is configured.");
    }
  }
