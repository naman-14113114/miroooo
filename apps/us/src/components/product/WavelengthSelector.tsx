...
import { wavelengths as modes } from "@/data/productSections";
...
export function WavelengthSelector() {
  ...
  return (
    <section className="store-section overflow-hidden bg-[var(--ink)] py-14 text-[var(--cream)] md:py-24">
      <div
        className="store-glow -left-40 top-1/2 h-[500px] w-[500px] -translate-y-1/2 transition-colors duration-700"
            ...
            <div className="relative w-full h-full overflow-hidden rounded-2xl">
              <video
                src={productMediaAsset(
                  "banner-collage.png",
                  "miroooo-electric-toothbrush-x2",
                  "images",
                )}
                ...
                <li key={wavelength.name}>
                  <button
                    className={`flex w-full items-center gap-4 border-b border-[rgba(247,241,232,.1)] py-2 text-left transition-all duration-300 hover:pl-3 group ${
                      isActive
                        ? "bg-[rgba(247,241,232,.03)] pl-3 border-l-2 border-l-[var(--gold)]"
                        : ""
                    }`}
                  ...
                  >
                    <span
                      ...
                      style={{
                        ...
                        boxShadow: isActive
                          ? `0 0 10px ${wavelength.color}`
                          : "none",
                      }}
                    />
                    <span className="store-display w-20 uppercase text-[rgba(247,241,232,.6)]">
                      {wavelength.nm}
                    </span>
                    <span
                      className={`store-display text-xl transition-colors duration-300 ${
                        isActive
                          ? "text-[var(--gold)]"
                          : "text-[var(--cream)] group-hover:text-[var(--gold)]"
                      }`}
                    >
                      {wavelength.name}
                    </span>
                    <span className="store-display ml-auto hidden text-sm text-[rgba(247,241,232,.6)] sm:block">
