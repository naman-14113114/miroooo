...
const KLAVIYO_UK_POPUP_FORM_ID = process.env.NEXT_PUBLIC_KLAVIYO_POPUP_FORM_ID?.trim();
...
const klaviyoNodeSelector = '[aria-modal="true"], [role="dialog"], [data-testid="POPUP"], div[class*="kl-private-reset-css"], .needsclick[class*="kl-private-reset-css"]';
...
function isBlockingKlaviyoModal(node: HTMLElement) {
    ...
    const modalAncestor = node.closest<HTMLElement>('[aria-modal="true"], [role="dialog"], [data-testid="POPUP"]');
    const container = modalAncestor ?? node;
    const containerRect = container.getBoundingClientRect();
    const containerStyle = window.getComputedStyle(container);
    const isFixedLayer = style.position === "fixed" ||
        containerStyle.position === "fixed" ||
        style.position === "sticky" ||
        containerStyle.position === "sticky";
    const coversModalArea = containerRect.width >= Math.min(window.innerWidth * 0.62, 420) &&
        containerRect.height >= Math.min(window.innerHeight * 0.28, 260);
    const looksLikeOfferModal = (text.includes("WELCOME") ||
...
function guardKlaviyoScrollLock() {
    ...
    const removeStaleDollarPopupNodes = () => {
        document
            ...
            const popup = node.closest<HTMLElement>('[aria-modal="true"], [role="dialog"], [data-testid="POPUP"]') ??
                node;
            popup.remove();
        });
    };
    const removeVisibleOfferPopups = () => {
        document
            ...
            .forEach((node) => {
            const text = node.textContent || "";
            if (!text.includes(`${staleDollarOfferLabel} WELCOME`) &&
    ...
    const hasVisibleBlockingKlaviyoModal = () => Array.from(document.querySelectorAll<HTMLElement>(klaviyoNodeSelector)).some(isBlockingKlaviyoModal);
    ...
    const handleDismissClick = (event: MouseEvent | PointerEvent) => {
        ...
        const clickedClose = Boolean(target.closest('[aria-label*="close" i], .klaviyo-close-form'));
...
export function KlaviyoAnalytics() {
    ...
    useEffect(() => {
        ...
        const loadKlaviyo = () => {
            if (document.querySelector("script[data-store-klaviyo='true']")) {
                return;
            }
            const script = document.createElement("script");
            script.async = true;
            script.dataset.storeKlaviyo = "true";
            script.src = `https://static.klaviyo.com/onsite/js/klaviyo.js?company_id=${encodeURIComponent(KLAVIYO_COMPANY_ID)}`;
    ...
    useEffect(() => {
        ...
        function handleStartedCheckout(event: Event) {
            ...
            pushKlaviyo([
                ...
                "Started Checkout",
                {
                    $event_id: `uk-store-${Date.now()}`,
                    $value: typeof detail?.totals?.totalCents === "number"
                        ? detail.totals.totalCents / 100
                        : items.reduce((total, item) => total + item.RowTotal, 0),
                    ItemNames: items.map((item) => item.ProductName),
                    CheckoutURL: toAbsoluteUrl("/cart"),
                    Categories: Array.from(new Set(items.flatMap((item) => item.ProductCategories))),
                    Items: items,
                    Market: market.marketLabel,
                    SourceSite: marketHost,
                },
        ...
        window.addEventListener("store:add-to-cart", handleAddToCart);
        window.addEventListener("store:started-checkout", handleStartedCheckout);
        return () => {
            window.removeEventListener("store:add-to-cart", handleAddToCart);
            window.removeEventListener("store:started-checkout", handleStartedCheckout);
        };
    }, []);
    return null;
}
