...
export function GiftBundle({ product }: { product: Product }) {
  ...
  useEffect(() => {
    fetch(
      "/media/products/miroooo-electric-toothbrush-x2/images/lottieflow-ecommerce-14-19-aa8e50-easey.json",
    )
      .then((res) => res.json())
    ...
    <div>
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        ...
        <div className="flex flex-nowrap items-center gap-x-1 sm:gap-x-1.5 text-[9.5px] sm:text-[13px] text-[var(--muted)]">
          <span className="whitespace-nowrap tracking-tight sm:tracking-normal">
            or{" "}
            <strong className="store-display text-[10px] sm:text-[14px] font-medium text-[var(--plum)]">
              4
            </strong>{" "}
            interest-free payments of{" "}
            <strong className="store-display text-[10px] sm:text-[14px] font-semibold text-[var(--plum)]">
      ...
      <ul className="mt-4 mb-4 flex flex-col gap-1.5 pl-1.5">
        {(product.keyBenefits ?? product.highlights).map((benefit, index) => {
          ...
          return (
            <li
              className="flex items-center gap-3 text-[14.5px] font-normal leading-normal text-[var(--plum)] store-display"
              key={benefit}
            >
        ...
        <div className="flex items-center justify-between gap-2 sm:gap-5">
          <div>
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <p className="store-eyebrow text-[var(--gold)] m-0 leading-none flex items-center h-5 sm:h-7 font-bold text-[10px] sm:text-xs">
                FREE DELIVERY
              </p>
            </div>
            <p className="store-display mt-1.5 text-base sm:text-2xl text-[var(--plum)] font-normal leading-none whitespace-nowrap">
              {deliveryDate || "soon"}
            </p>
          </div>
          <div className="text-right">
            <p className="store-eyebrow text-[var(--gold)] whitespace-nowrap text-[9px] sm:text-[11px] tracking-tight sm:tracking-normal">
              {hasGifts ? "ORDER WITHIN" : "ORDER TODAY"}
            </p>
            <p className="store-display mt-1.5 text-xl sm:text-[2.2rem] font-normal text-[var(--plum)] leading-none">
              {timer}
            </p>
          </div>
        </div>
      ...
      {hasGifts ? (
        <section className="mt-8" id="free-gifts">
          <div className="text-center mb-8 flex flex-col items-center">
            <h3 className="store-display text-3xl font-medium text-[var(--plum)]">
              Special Spring Sale
            </h3>
            <p className="store-mono mt-2 inline-flex items-center justify-center gap-1.5 flex-wrap rounded bg-[rgba(184,149,86,.15)] px-3 py-1 text-xs sm:text-sm font-bold tracking-widest text-[var(--plum)]">
              <span className="store-display text-sm sm:text-base font-extrabold normal-case text-[var(--plum)]">
            ...
            {product.gifts.map((gift) => (
              ...
              >
                ...
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-2 sm:px-3 py-1 bg-[var(--card)] border border-[rgba(58,31,61,.22)] rounded-full flex items-center gap-1 sm:gap-1.5 shadow-[0_2px_8px_rgba(58,31,61,0.06)] whitespace-nowrap">
                  ...
                  <span className="line-through text-xs sm:text-sm text-[var(--muted)] store-display font-semibold leading-none">
                ...
                {/* Gift Label / Title */}
                <p className="store-display mt-0.5 md:mt-1 text-base md:text-lg font-semibold text-[var(--plum)] leading-snug">
                  {gift.name}
                </p>
              </Link>
            ))}
        ...
        >
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
            Includes the Miroooo X2 toothbrush, 2 premium brush heads, travel case, USB-C cable, and user manual for a complete sonic oral care routine.
          </p>
        </section>
      )}
