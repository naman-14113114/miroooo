...
export function StickyAddToCart({ product }: { product: Product }) {
  ...
  useEffect(() => {
    fetch("/media/products/miroooo-electric-toothbrush-x2/images/lottieflow-ecommerce-14-8-f6ede2-cart.json")
      .then((res) => res.json())
  ...
  useEffect(() => {
    document.documentElement.classList.add("store-mask-sticky-cta");

    return () => {
      document.documentElement.classList.remove("store-mask-sticky-cta");
    };
  }, [product.template]);
    ...
    >
      ...
      >
        ...
        <Button
          ...
          className="store-cart-wipe min-h-11 w-full flex-none px-3 text-[11px] sm:min-h-12 sm:w-auto sm:px-6 sm:text-sm whitespace-nowrap"
          onClick={() => {
            addProduct(product);
            router.push("/cart");
          }}
        >
          {cartIconData ? (
            <div className="store-sticky-cart-icon flex h-5 w-5 flex-shrink-0 items-center justify-center">
