...
const variantClasses: Record<ButtonVariant, string> = {
  primary:
    "store-cart-wipe border border-[var(--plum)] bg-[var(--plum)] text-[var(--cream)] shadow-[0_14px_30px_-18px_rgba(58,31,61,.8)] hover:border-[var(--gold)]",
  ghost:
...
function ButtonContent({ children }: { children: ReactNode }) {
  return (
    <span className="store-cart-wipe-content inline-flex items-center justify-center gap-2">
      {children}
    </span>
  );
}
