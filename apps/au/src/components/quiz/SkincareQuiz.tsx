...
export function SkincareQuiz() {
                ...
                : selected.includes(optionValue)
                    ...
                    : [
                        ...selected.filter((value) => {
                            const selectedOption = question.options.find((candidate) => candidate.value === value);
    ...
    return (<div className="min-h-[620px] scroll-mt-24 rounded-[18px] border border-[var(--border)] bg-[var(--card)] p-5 shadow-[0_30px_80px_-60px_rgba(58,31,61,.7)] sm:p-8 lg:p-10" ref={quizPanel}>
      {stage === "intro" ? (<QuizIntro savedQuiz={savedQuiz} onResume={resumeQuiz} onStart={startFresh}/>) : null}

      {stage === "questions" ? (<QuizQuestionStep answers={answers} currentQuestion={currentQuestion} onBack={() => {
                setQuestionIndex((index) => Math.max(0, index - 1));
                scrollToQuizPanel();
            }} onContinue={continueQuiz} onReset={resetQuiz} onSelect={selectOption} questionIndex={questionIndex}/>) : null}

      {stage === "analyzing" ? <QuizAnalyzing /> : null}

      {stage === "results" && result ? (<QuizResults onReset={resetQuiz} result={result}/>) : null}
    </div>);
}
function QuizIntro({ savedQuiz, onResume, onStart, }: {
    savedQuiz: SavedQuiz | null;
    onResume: () => void;
    onStart: () => void;
}) {
    return (<div className="flex min-h-[540px] flex-col justify-center">
      <p className="store-eyebrow">60-second assessment</p>
      <h2 className="store-display mt-4 text-[2.8rem] leading-[1.02] text-[var(--plum)] md:text-[4rem]">
        Build your custom <em className="store-italic">routine</em>.
      </h2>
      <p className="store-copy mt-5 max-w-xl">
        Discover the exact LED light therapy and daily oral care habits you need
        for your specific concerns.
      </p>

      {savedQuiz ? (<div className="mt-8 rounded-[14px] border border-[rgba(180,145,76,.38)] bg-[rgba(180,145,76,.09)] p-5">
          <p className="store-mono text-[var(--gold)]">Routine in progress</p>
        ...
        </div>) : (<Button className="mt-8 self-start" onClick={onStart}>
          Start assessment
          <ArrowRight size={16}/>
        </Button>)}

      <div className="mt-12 grid gap-3 border-t border-[var(--border)] pt-6 sm:grid-cols-3">
        {["5 focused questions", "Instant routine", "Private by design"].map((item) => (<div className="flex items-center gap-2 text-sm text-[var(--muted)]" key={item}>
...
function QuizQuestionStep({ answers, currentQuestion, onBack, onContinue, onReset, onSelect, questionIndex, }: {
    answers: QuizAnswers;
    ...
    return (<div>
      ...
      <h2 className="store-display mt-9 text-[2.25rem] leading-[1.06] text-[var(--plum)] md:text-[3.2rem]">
      ...
      <div aria-label={currentQuestion.title} className={cn("mt-8 grid gap-3", currentQuestion.id === "concern" || currentQuestion.id === "age"
            ? "sm:grid-cols-2"
        ...
        {currentQuestion.options.map((option) => {
            const selected = Array.isArray(answer)
                ...
                : answer === option.value;
            return (<button aria-checked={selected} className={cn("group flex min-h-16 w-full items-center gap-4 rounded-[14px] border px-4 py-4 text-left transition duration-200 ease-out focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--gold)]", selected
                    ? "border-[var(--gold)] bg-[rgba(180,145,76,.1)] shadow-[0_12px_24px_-22px_rgba(58,31,61,.8)]"
                    : "border-[var(--border)] bg-[var(--cream)] hover:border-[rgba(180,145,76,.62)] hover:bg-[rgba(180,145,76,.05)]")} key={option.value} onClick={() => onSelect(currentQuestion, option.value)} role={currentQuestion.selection === "single" ? "radio" : "checkbox"} type="button">
              <span className={cn("grid h-6 w-6 shrink-0 place-items-center border transition", currentQuestion.selection === "single"
                    ? "rounded-full"
                    : "rounded-[7px]", selected
              ...
              <span>
                ...
                {option.description ? (<span className="mt-1.5 block text-xs leading-5 text-[var(--muted)]">
                    {option.description}
                  </span>) : null}
              </span>
            </button>);
      ...
      <div className="mt-8 flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] pt-6">
        <Button className={questionIndex === 0 ? "invisible" : ""} onClick={onBack} tabIndex={questionIndex === 0 ? -1 : 0} variant="ghost">
          <ArrowLeft size={16}/>
          Back
        </Button>
...
function QuizAnalyzing() {
    return (<div aria-live="polite" className="flex min-h-[540px] flex-col items-center justify-center text-center" role="status">
      ...
      <p className="store-eyebrow mt-8">Analyzing data</p>
      <h2 className="store-display mt-3 text-4xl text-[var(--plum)]">
        Formulating your routine.
      </h2>
      <p className="store-copy mt-3 max-w-md text-sm">
        Matching your concerns with a practical clinical-grade home ritual.
      </p>
    </div>);
}
function QuizResults({ onReset, result, }: {
    onReset: () => void;
    result: QuizResult;
}) {
    return (<div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="store-eyebrow">Your skin dossier</p>
          <h2 className="store-display mt-3 text-[2.8rem] leading-none text-[var(--plum)] md:text-[4rem]">
            Your routine, <em className="store-italic">revealed</em>.
          </h2>
        </div>
        <button className="inline-flex items-center gap-2 text-xs font-semibold text-[var(--muted)] transition hover:text-[var(--plum)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--gold)]" onClick={onReset} type="button">
          <RotateCcw size={14}/>
      ...
      <div className="mt-6 rounded-[14px] border border-[rgba(180,145,76,.34)] bg-[rgba(180,145,76,.08)] p-4">
        <p className="store-mono text-[var(--gold)]">Profile</p>
      ...
      {result.pregnancyWarning ? (<div className="mt-4 flex gap-3 rounded-[14px] border border-[rgba(180,145,76,.48)] bg-[rgba(180,145,76,.12)] p-4">
      ...
      <div className="mt-6 space-y-3">
        {result.routine.map((step) => (<article className={cn("grid gap-3 rounded-[14px] border p-5 sm:grid-cols-[56px_1fr]", step.highlighted
                ? "border-[rgba(180,145,76,.55)] bg-[var(--plum)] text-[var(--cream)]"
                : "border-[var(--border)] bg-[var(--cream)]")} key={step.number}>
            <p className={cn("store-display text-4xl", step.highlighted ? "text-[var(--gold)]" : "text-[var(--gold)]")}>
              {step.number}
            </p>
            <div>
              <h3 className={cn("store-display text-2xl", step.highlighted ? "text-[var(--cream)]" : "text-[var(--plum)]")}>
                {step.title}
              </h3>
              {step.highlighted ? (<p className="store-mono mt-2 text-[var(--gold)]">
                  Your prescribed setting: {result.ledSetting}
                </p>) : null}
              <p className={cn("mt-3 text-sm leading-6", step.highlighted
      ...
      <div className="mt-6 rounded-[14px] bg-[var(--blush)] p-5 text-center sm:p-7">
        ...
        <h3 className="store-display mt-3 text-3xl text-[var(--plum)]">
      ...
      <p className="mt-5 text-xs leading-5 text-[var(--muted)]">
        This quiz provides general oral care guidance and is not medical advice.
