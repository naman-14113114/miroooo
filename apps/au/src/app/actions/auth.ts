...
function missingSupabaseState(): AuthActionState {
    return {
        status: "error",
        message: "Supabase is not configured yet. Add the Supabase environment variables and run the database migration.",
    };
}
function parseAction<T extends z.ZodTypeAny>(schema: T, formData: FormData) {
...
export async function signUpAction(_state: AuthActionState, formData: FormData): Promise<AuthActionState> {
    ...
    const { data, error } = await supabase.auth.signUp({
        ...
        password: parsed.data.password,
        options: {
            emailRedirectTo: `${origin}/auth/callback?next=${encodeURIComponent(parsed.data.redirectTo || "/my-profile")}`,
            data: {
    ...
    return {
        status: "success",
        message: `Account created. Check your email to confirm your ${BRAND_NAME} account, then sign in.`,
    };
}
