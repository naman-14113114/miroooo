...
export const metadata: Metadata = {
    ...
    description: `Your ${BRAND_NAME} order confirmation.`,
    robots: {
        index: false,
        follow: false,
    },
...
export default async function OrderConfirmationPage({ params, }: OrderConfirmationPageProps) {
    ...
    if (error || !order) {
        ...
        <div className="store-wrap max-w-2xl text-center">
          ...
          <h1 className="store-heading mt-5">Order not found.</h1>
    ...
    return (<section className="store-section bg-[var(--cream)] py-16 md:py-24">
      <div className="store-wrap">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="store-eyebrow mt-5">Sale recorded</p>
          <h1 className="store-heading mt-3">Thank you, {order.customer_full_name}.</h1>
          <p className="store-copy mx-auto mt-5 max-w-2xl">{`
            Your ${BRAND_NAME} order has been recorded as `}{order.order_number}. A real
        ...
        <div className="mx-auto mt-12 grid max-w-5xl gap-8 lg:grid-cols-[1fr_360px]">
          <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-5 md:p-6">
            <div className="flex flex-col justify-between gap-4 border-b border-[var(--border)] pb-5 md:flex-row md:items-center">
              <div>
                <p className="store-mono text-[var(--gold)]">Order</p>
                <h2 className="store-display mt-2 text-3xl text-[var(--plum)]">
                  {order.order_number}
                </h2>
              ...
              <p className="store-display text-4xl text-[var(--plum)]">
          ...
          <aside className="space-y-5">
            <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-6">
              <p className="store-mono text-[var(--gold)]">Customer</p>
              ...
              {order.customer_phone ? (<p className="mt-2 text-sm text-[var(--muted)]">
                  {order.customer_phone}
                </p>) : null}
            </div>

            <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-6">
              <p className="store-mono text-[var(--gold)]">Summary</p>
              <div className="mt-4 space-y-3 text-sm">
                <SummaryRow label="Subtotal" value={formatMoney(order.subtotal_cents)}/>
                <SummaryRow label="Shipping" value="Free"/>
                <SummaryRow label="Savings" value={`-${formatMoney(order.savings_cents)}`}/>
                <SummaryRow label="Gift value" value={formatMoney(order.gift_value_cents)}/>
              </div>
            </div>
          </aside>
        </div>
...
function OrderSetupNotice({ orderNumber }: {
    orderNumber: string;
}) {
    return (<section className="store-section bg-[var(--cream)] py-20 md:py-28">
      <div className="store-wrap max-w-2xl text-center">
        <PackageCheck className="mx-auto text-[var(--gold)]" size={46}/>
        <h1 className="store-heading mt-5">Order storage needs setup.</h1>
        <p className="store-copy mx-auto mt-5 max-w-lg">
