...
const pageDescription = "A Australia buyer's guide to choosing the best LED face mask for red light therapy, blue light acne routines, anti-ageing skincare, near-infrared support, neck coverage, and value.";
const checkpoints = [
    {
    ...
    },
    {
        ...
        copy: `A mask that only covers the face can leave your neck out of the routine. ${BRAND_NAME} covers the face and neck together.`,
    },
    {
...
const comparisonRows = [
    ...
    ["Australia offer", `${formatMoney(storeToothbrush.priceCents, storeToothbrush.currency)} launch price with free glow kit`],
        ...
        images: [
            {
                ...
                width: 1200,
                height: 900,
                alt: `${BRAND_NAME} LED Mask home light therapy routine`,
            },
        ],
    },
    twitter: {
...
export default function BestLedFaceMaskUkPage() {
    ...
    return (<>
      {jsonLd.map((schema, index) => (<script dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} key={index} type="application/ld+json"/>))}

      <section className="store-section bg-[var(--plum)] py-16 text-[var(--cream)] md:py-24">
        <div className="store-wrap grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
          <div>
            <p className="store-mono text-[var(--gold)]">Australia LED mask guide</p>
            <h1 className="store-display mt-5 text-5xl leading-none md:text-7xl">
          ...
          <figure className="relative min-h-[460px] overflow-hidden rounded-[34px] bg-[var(--ink)]">
            <Image alt={`${BRAND_NAME} LED Mask used at home in Australia`} className="object-cover" fill priority sizes="(min-width: 1024px) 48vw, 100vw" src="/images/products/buudy-led-mask/09-buudy-led-mask-home-spa.webp"/>
      ...
      <section className="store-section bg-[var(--cream)] py-16 md:py-24">
        <div className="store-wrap">
          <div className="max-w-3xl">
            <p className="store-mono text-[var(--gold)]">Quick answer</p>
            <h2 className="store-display mt-4 text-4xl leading-none text-[var(--plum)] md:text-6xl">
          ...
          <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {checkpoints.map((item) => (<article className="rounded-[24px] border border-[var(--border)] bg-[var(--card)] p-6" key={item.title}>
                <CheckCircle2 className="text-[var(--gold)]" size={22}/>
                <h3 className="store-display mt-5 text-2xl leading-tight text-[var(--plum)]">
                  {item.title}
                </h3>
      ...
      <section className="store-section bg-[rgba(241,223,210,.5)] py-16 md:py-24" id="comparison">
        <div className="store-wrap grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div>
            <p className="store-mono text-[var(--gold)]">{`Why ${BRAND_NAME}`}</p>
            <h2 className="store-display mt-4 text-4xl leading-none text-[var(--plum)] md:text-6xl">
              Built for the Australia buyer comparing premium LED masks.
            </h2>
            <p className="mt-5 text-base leading-8 text-[var(--muted)]">{`
              The right LED mask should remove trade-offs. ${BRAND_NAME} is positioned
            ...
            <div className="mt-7 flex items-start gap-4 rounded-[22px] bg-[var(--plum)] p-5 text-[var(--cream)]">
              ...
              <p className="text-sm leading-7 text-[rgba(247,241,232,.76)]">{`
                ${BRAND_NAME} is a beauty and wellness device, not a medical treatment.
          ...
          <div className="overflow-hidden rounded-[30px] border border-[var(--border)] bg-[var(--card)]">
            {comparisonRows.map(([label, value]) => (<div className="grid gap-2 border-b border-[var(--border)] px-5 py-4 last:border-b-0 md:grid-cols-[190px_1fr]" key={label}>
                <p className="store-mono text-[var(--gold)]">{label}</p>
      ...
      <section className="store-section bg-[var(--cream)] py-16 md:py-24">
        <div className="store-wrap grid gap-10 lg:grid-cols-[1.05fr_.95fr] lg:items-center">
          <figure className="relative min-h-[500px] overflow-hidden rounded-[34px] bg-[var(--blush)]">
            <Image alt={`${BRAND_NAME} LED Mask front product view`} className="object-cover" fill sizes="(min-width: 1024px) 52vw, 100vw" src="/images/products/buudy-led-mask/01-buudy-led-mask-front.webp"/>
          </figure>
          <div>
            <p className="store-mono text-[var(--gold)]">Ready to buy</p>
            <h2 className="store-display mt-4 text-4xl leading-none text-[var(--plum)] md:text-6xl">
        ...
        ].map((item) => (<li className="flex items-center gap-3 text-[var(--plum)]" key={item}>
