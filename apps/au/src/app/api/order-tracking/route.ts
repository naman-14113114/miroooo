...
function findEvents(value: unknown, depth = 0): TrackingEvent[] {
    ...
    if (Array.isArray(value)) {
        const events = value
            ...
            if (!isRecord(item))
                return null;
            const label = findDeep(item, ["status", "title", "checkpoint_status"], 0) ||
...
function errorMessage(code: string) {
    if (code === "email_not_matched") {
        return `That email does not match the order number. Use the exact email from the ${BRAND_NAME} checkout confirmation.`;
    }
    if (code === "order_not_found" || code === "not_found") {
        return `No matching ${BRAND_NAME} order was found. Check the order number from your confirmation email and try again.`;
    }
...
export async function POST(request: NextRequest) {
  if (!plusbaseStoreUrl) {
    return NextResponse.json({ code: "TRACKING_UNAVAILABLE", message: "Order tracking is unavailable until PlusBase is configured." }, { status: 503 });
  }
    const body = (await request.json().catch(() => ({}))) as AnyRecord;
    const orderName = normalizeOrder(body.orderNumber ?? body.order_name);
    const email = asText(body.email).toLowerCase();
    if (!orderName || !email) {
        return NextResponse.json({ status: "error", message: "Enter both your order number and email." }, { status: 400 });
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    try {
        lookup = await fetch(`${plusbaseStoreUrl}/api/checkout/order-status/url.json`, {
            ...
            headers: {
                ...
                "user-agent": "Mozilla/5.0 StoreOrderTracking/1.0",
        ...
        return NextResponse.json({ status: "error", message: `We could not reach ${BRAND_NAME} tracking right now. Please try again.` }, { status: 502 });
    }
    const lookupJson = (await lookup.json().catch(() => ({}))) as AnyRecord;
    if (!lookup.ok) {
        const code = asText(lookupJson.error) || asText(lookupJson.code) || "not_found";
        return NextResponse.json({ status: "not_found", code, message: errorMessage(code) }, { status: lookup.status === 404 ? 404 : 502 });
    }
    ...
    if (!token || !accessKey) {
        return NextResponse.json({ status: "error", message: `${BRAND_NAME} found the order, but did not return a secure status token.` }, { status: 502 });
    }
    const officialOrderUrl = `${plusbaseStoreUrl}/orders/${encodeURIComponent(token)}`;
    const info = await fetch(`${plusbaseStoreUrl}/api/checkout/order-status/${encodeURIComponent(token)}/next/info.json?key=${encodeURIComponent(accessKey)}`, {
        method: "GET",
        cache: "no-store",
        headers: {
            accept: "application/json",
    ...
    const shipmentStatus = findDeep(infoRoot, ["fulfillment_status", "shipment_status", "shipping_status", "status", "financial_status"]) ||
        "Order found";
    ...
    return NextResponse.json({
        ...
        message: trackingAvailable
            ? `Real tracking details found from ${BRAND_NAME} checkout.` : "Order found. Carrier tracking has not been assigned yet, or it is still updating in the carrier system.",
    });
}
