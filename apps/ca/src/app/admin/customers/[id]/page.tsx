...
export const metadata: Metadata = {
    ...
    description: `${BRAND_NAME} admin customer detail.`,
    robots: {
        index: false,
        follow: false,
    },
    ...
    return (<section className="store-section bg-[var(--cream)] py-16 md:py-24">
      <div className="store-wrap">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
          <div>
            <p className="store-eyebrow">Customer</p>
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-3">
              <DetailCard icon={<Mail size={18}/>} label="Email" value={customer.email}/>
              <DetailCard icon={<Phone size={18}/>} label="Phone" value={customer.phone || "Not provided"}/>
              <DetailCard icon={<ShoppingBag size={18}/>} label="Total spend" value={formatMoney(totalSpend)}/>
            </div>

            <section className="mt-8 rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-5 md:p-6">
              <p className="store-mono text-[var(--gold)]">Order history</p>
              <div className="mt-5 space-y-3">
                {orders.length ? (orders.map((order) => (<Link className="flex flex-col justify-between gap-3 rounded-2xl border border-[var(--border)] bg-[var(--cream)] p-4 transition hover:border-[var(--gold)] md:flex-row md:items-center" href={`/admin/orders/${order.order_number}`} key={order.id}>
                      <span>
                    ...
                    </Link>))) : (<p className="text-sm text-[var(--muted)]">
          ...
          <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
            <section className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-6">
              <p className="store-mono text-[var(--gold)]">Profile id</p>
            ...
            <section className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-6">
              <p className="store-mono text-[var(--gold)]">Shipping</p>
            ...
            <section className="rounded-[2rem] border border-[var(--border)] bg-[var(--card)] p-6">
              <p className="store-mono text-[var(--gold)]">Account signals</p>
              <div className="mt-4 space-y-3 text-sm">
                <DetailRow label="Marketing opt-in" value={customer.marketing_opt_in ? "Yes" : "No"}/>
                <DetailRow label="Created" value={dateFormatter.format(new Date(customer.created_at))}/>
                <DetailRow label="Latest order" value={latestOrder?.order_number ?? "None"}/>
              </div>
            </section>
          </aside>
        </div>
...
function AdminNotice({ message }: {
    ...
    return (<section className="store-section bg-[var(--cream)] py-20 md:py-28">
      <div className="store-wrap max-w-2xl text-center">
        ...
        <h1 className="store-heading mt-4">Customer unavailable.</h1>
