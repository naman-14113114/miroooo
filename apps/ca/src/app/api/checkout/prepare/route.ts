...
const plusbaseOrigin = process.env.PLUSBASE_ORIGIN?.trim() ?? "";
const maskProductId = Number(process.env.PLUSBASE_MASK_PRODUCT_ID);
const maskVariantId = Number(process.env.PLUSBASE_MASK_VARIANT_ID);
const torchProductId = Number(process.env.PLUSBASE_GIFT_PRODUCT_ID);
const torchVariantId = Number(process.env.PLUSBASE_GIFT_VARIANT_ID);
...
function hasPlusbaseConfiguration() {
  return Boolean(
    plusbaseOrigin &&
      Number.isFinite(maskProductId) &&
      maskProductId > 0 &&
      Number.isFinite(maskVariantId) &&
      maskVariantId > 0 &&
      Number.isFinite(torchProductId) &&
      torchProductId > 0 &&
      Number.isFinite(torchVariantId) &&
      torchVariantId > 0,
  );
}
...
export async function POST(request: NextRequest) {
  ...
  if (!hasPlusbaseConfiguration()) {
    return NextResponse.json(
      {
        code: "CHECKOUT_UNAVAILABLE",
        message: "Checkout is unavailable until PlusBase is configured.",
      },
      { status: 503 },
    );
  }
