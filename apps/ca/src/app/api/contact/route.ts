...
export async function POST(request: Request) {
    ...
    if (!accessKey) {
        return jsonResponse({
            success: false,
            message: "The contact form is not configured yet. Please email support@buudy.com directly.",
            code: "CONFIG_MISSING",
        }, 503);
    }
    try {
        const response = await fetch(web3FormsEndpoint, {
            ...
            body: JSON.stringify({
                ...
                subject: "" + "[" + BRAND_NAME + " Contact] " + (subject || "New support request") + "",
        ...
        return jsonResponse({
            success: false,
            message: "We could not send your message right now. Please email support@buudy.com directly.",
            code: "NETWORK_ERROR",
        }, 502);
    }
}
