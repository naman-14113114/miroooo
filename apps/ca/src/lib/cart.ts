...
function findProductForLine(line: CartLine) {
    return (getProductById(line.productId) ??
...
export function upsertProductCartLines(lines: CartLine[], product: Product, quantity: number) {
...
export function calculateCartTotals(lines: CartLine[]) {
    ...
    const subtotalCents = productLines.reduce((total, line) => total + line.unitPriceCents * line.quantity, 0);
    const compareAtCents = productLines.reduce((total, line) => total + (line.compareAtCents ?? line.unitPriceCents) * line.quantity, 0);
...
export function getDisplayLines(lines: CartLine[]): CartLine[] {
    const hasMaskProduct = lines.some((line) => line.productId === "miroooo-electric-toothbrush-x2" && line.type === "product");
    if (!hasMaskProduct) {
        return lines;
    }
    return lines
        .map((line) => {
        if (line.productId === "miroooo-electric-toothbrush-x2" && line.type === "product") {
            return {
                ...line,
                title: `${BRAND_NAME} Electric Toothbrush X2 + Travel Case`,
                subtitle: "Premium sonic toothbrush with the travel case included for protected storage and portability.",
                image: "/images/products/miroooo-electric-toothbrush-x2/05-travel-case.png",
            };
        }
        return line;
    })
        .filter((line) => {
        if (line.productId === "miroooo-electric-toothbrush-x2" && line.type === "gift") {
            const giftId = line.id.split(":")[1];
            if (giftId === "premium-travel-case" || giftId === "skincare-ebook") {
                return false;
            }
        }
        return true;
