...
for (const file of files) {
  ...
  // Treatment Time
  content = content.replace(
    /(<strong key="4" className="store-display font-bold text-sm md:text-base text-\[var\(--muted\)]">)3 MINS(<\/strong>)/g,
    '$110 MINS$2'
  );
  ...
  if (file.includes('/uk/')) {
    ...
    content = content.replace(
      /(<span key="4" className="store-display text-base md:text-lg text-\[var\(--muted\)]">)\$555(<\/span>)/,
      '$1$399$2'
    );
  } else if (file.includes('/au/')) {
    content = content.replace(
      /(<span key="4" className="store-display text-base md:text-lg text-\[var\(--muted\)]">)AU\$555(<\/span>)/,
      '$1AU$399$2'
    );
  }
